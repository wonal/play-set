...
namespace Set.ApiTests
{
    class WinAndResetTests
    {
        ...
        {
            var seed = new NewGameDTO { IsDaily = true, UserLocalTime = new DateTime(2022, 5, 9).ToShortTimeString() };
        ...
        {
            var seed = new NewGameDTO { IsDaily = false, UserLocalTime = DateTime.Now.ToString() };
            StringContent postContent = TestUtilities.ObjToStringContent(seed);
            HttpClient client = TestUtilities.GetHttpClient();
